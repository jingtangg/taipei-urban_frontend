...
開發過程使用生成式 AI（Claude）輔助：**發想**（問題框架定位，如將敘事方向由「消防車現場救災導航」修正為「規劃人員辦公室決策前分析工具」）、**開發**（程式碼撰寫與 API 架構設計）、**驗證**（法規條文依據核對、跨縣市資料邏輯交叉檢查）。元件運行時完全不依賴 AI，但輸出的標準 GeoJSON 格式設計上支援 AI Agent 直接調用。